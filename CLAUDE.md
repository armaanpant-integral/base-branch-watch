...
| `osascript display notification` as the permanent notification mechanism if you ever want actionable buttons | AppleScript notifications are fire-and-forget banners; they cannot register interactive notification categories/actions — that capability is exclusively `UNUserNotificationCenter`, which requires a code-signed app | Keep `osascript` for v1 (matches current no-signed-bundle constraint — this is correct as-is); migrate to `pyobjc-framework-UserNotifications` + `UNUserNotificationCenter` only when/if you build the signed `.app` for the deferred "one-click pull" feature |
